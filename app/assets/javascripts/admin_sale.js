...
//solo llena el campo del valor unidad 
function llenar_campos(elemento){
  var myObj = JSON.parse('{"p": 5}');
    ...
    // productsCredit = $.parseJSON($(elemento).parent().parent().find('#product').val());
productSale = JSON.stringify($(elemento).val());
    console.log(productSale.serial);
    // productsCredit = $.parseJSON($(elemento).val());
    $(elemento).parent().parent().find("input#unit_value").val(13);
  // if (productsInventory != null) {
      // $("input.inventory_product_id").each(function(){
          var input = $(elemento);
        // if (parseInt(input.val()) == parseInt(productsInventory.id)){
                  // input.parent().parent().find("input#unit_value").val(productsCredit.serial);
          // }
      // });
  // }
...
function busquedaProducto(barcode){
    $(barcode).autocomplete({
      source: location.protocol + '//' + location.host + "/sales/autocomplete.json",
      minLength: 2,
      select: function( event, ui ) {
        $(barcode).parent().parent().find("#product_id").val(ui.item.id);
        $(barcode).parent().parent().find("#unit_value").val(ui.item.vale_sale);
        $(barcode).parent().parent().find("#inventory_fields").val(13);
        // debugger;
      }        
...
// suma el valor de la venta completa
function sumaValorVenta(total){
    var sum = 0;
...
//calcula el valor de la venta por producto
